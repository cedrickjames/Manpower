...
$tableUsers="users";
$columnUsers= ['account_ID','user_id','full_name','user_password','department','approved',];
$fetchDataUsers = fetch_data_users($db, $tableUsers, $columnUsers);


function fetch_data_users($db, $tableUsers, $columnUsers){
 if(empty($db)){
  $msg= "Database connection error";
 }elseif (empty($columnUsers) || !is_array($columnUsers)) {
  $msg="columns Name must be defined in an indexed array";
 }elseif(empty($tableUsers)){
   $msg= "Table Name is empty";
}else{
$columnName = implode(", ", $columnUsers);
// $query = "SELECT * FROM `model`;";
// $idModelLine= $_SESSION['lineId'];
$Department = $_SESSION['department'];
